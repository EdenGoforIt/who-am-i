...
function ProjectLinks({
  ...
  return (
    <div className={`flex gap-4 flex-wrap ${justifyClass}`}>
      {project.githubUrl && (
        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <GitHubIcon />
        </a>
      )}
      {project.liveUrl && (
        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" aria-label="Live site" className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <ExternalLinkIcon />
        </a>
      )}
      {project.webUrl && (
        <a href={project.webUrl} target="_blank" rel="noopener noreferrer" aria-label="Website" className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <GlobeIcon />
        </a>
      )}
      {project.androidUrl && (
        <a href={project.androidUrl} target="_blank" rel="noopener noreferrer" aria-label="Google Play" className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <AndroidIcon />
        </a>
      )}
      {project.iosUrl && (
        <a href={project.iosUrl} target="_blank" rel="noopener noreferrer" aria-label="App Store" className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <AppleIcon />
        </a>
      )}
      {project.articles?.map((url, idx) => (
        <a key={url} href={url} target="_blank" rel="noopener noreferrer" aria-label={`Article ${idx + 1}`} className="text-slate-lighter transition-colors duration-300 hover:text-green">
          <ArticleIcon />
        </a>
      ))}
    </div>
        ...
        {other.map((project, i) => (
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="text-green">
